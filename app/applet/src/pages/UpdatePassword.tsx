...
export default function UpdatePassword() {
  ...
  useEffect(() => {
    // Optional: We can check if there's a session here, because Supabase will 
    // log the user in when they click the reset link in their email.
    // If not logged in, they can't update the password via updateUser.
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session) {
        // Just a warning, if the hash fragment hasn't been parsed yet it might still load soon
        // But let's rely on Supabase doing its thing.
      }
    });
  }, []);
